...
public:
    DescriptorImpl(std::vector<std::unique_ptr<PubkeyProvider>> pubkeys, std::unique_ptr<DescriptorImpl> script, std::unique_ptr<TaprootTree> taproot, const std::string& name) : m_pubkey_args(std::move(pubkeys)), m_script_arg(std::move(script)), m_taproot(std::move(taproot)), m_name(name) {}
...
bool InsertTaproot(const Span<const char>& sp, TaprootTree* node, FlatSigningProvider& out)
{
    ...
    while (it != sp.end()) {
        switch(*it) {
            case '{':
                return InsertTaproot(Span<const char>(it+1, sp.end()), node->GetInitLeftChild(), out);
            case ',': {
                if (*(it-1) == ')') {
                    auto desc_sp = Span<const char>(sp.begin(), it);
                    std::unique_ptr<DescriptorImpl> desc = ParseScript(desc_sp, ParseScriptContext::Tap, out);
                    if(!desc || !node->InsertLeaf(std::move(desc))) return false;
                }
                return InsertTaproot(Span<const char>(it+1, sp.end()), node->GetInitRightSibling(), out);
            }
            case '}': {
                if (*(it-1) == ')') {
                    auto desc_sp = Span<const char>(sp.begin(), it);
                ...
                if (!parent || !parent->IsFull()) return false;
                return InsertTaproot(Span<const char>(it+1, sp.end()), parent, out);
            }
        }
        ++it;
    }
...
std::unique_ptr<TapDescriptor> ParseTaproot(const Span<const char>& sp, std::unique_ptr<PubkeyProvider> internal_key, FlatSigningProvider& out)
{
...
//    if ((*sp.begin() != '{') || (*(sp.end()-1) != '}')) return nullptr;
    ...
    if(!InsertTaproot(sp, root.get(), out)) return nullptr;
...
std::unique_ptr<DescriptorImpl> ParseScript(Span<const char>& sp, ParseScriptContext ctx, FlatSigningProvider& out)
{
    auto expr = Expr(sp);
    if (Func("pk", expr)) {
        auto pubkey = ParsePubkey(expr, (ctx != ParseScriptContext::P2WSH && ctx != ParseScriptContext::Tap), out);
    ...
    if (ctx == ParseScriptContext::TOP && Func("tap", expr)) {
        ...
        auto internal_key = ParsePubkey(key_only.first, false, out);
        if (!internal_key) return nullptr;
        return ParseTaproot(key_only.second, std::move(internal_key), out);
    }
    return nullptr;
}
